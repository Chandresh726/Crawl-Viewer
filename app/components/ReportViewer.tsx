...
const HeaderSection = ({ url, metadata }: { url: string, metadata: any }) => {
...
const LinksSection = ({ links }: { links: { internal: string[], external: string[] } }) => {
  ...
  const renderLinks = (links: string[], limit: number, setLimit: (value: number) => void) => {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 shadow-sm border border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 text-center">Internal Links ({links.internal.length})</h3>
          <hr className="my-2" />
          {links.internal.length > 0 ? renderLinks(links.internal, internalLimit, setInternalLimit) : (
        ...
        <div className="bg-white p-6 shadow-sm border border-gray-200">
          ...
          <hr className="my-2" />
          {links.external.length > 0 ? renderLinks(links.external, externalLimit, setExternalLimit) : (
...
const StorageSection = ({ storage }: { storage: any }) => {
  ...
  const StorageData = ({ data, title, limit, setLimit }: { data: any, title: string, limit: number, setLimit: (value: number) => void }) => (
    <div className="bg-white p-6 shadow-sm border border-gray-200">
          ...
          {Object.entries(data).slice(0, limit).map(([key, value], i) => (
            <div key={i} className="border-b border-gray-100 pb-3 last:border-0 last:pb-0">
              ...
              <div className="mt-1 flex items-center justify-between">
                ...
                <button className="ml-2 text-blue-600 hover:underline text-xs cursor-pointer" onClick={() => handleCopy(typeof value === 'string' ? value : JSON.stringify(value), i)}>
                  {copiedIndex === i ? <IconCheck className="w-4 h-4 text-green-600" /> : <IconCopy className="w-4 h-4 hover:text-blue-800" />}
                </button>
              </div>
            </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <StorageData data={hasLocalStorage ? storage.localStorage : null} title="Local Storage" limit={localLimit} setLimit={setLocalLimit} />
        <StorageData data={hasSessionStorage ? storage.sessionStorage : null} title="Session Storage" limit={sessionLimit} setLimit={setSessionLimit} />
      </div>
    </div>
  );
};

const CookiesSection = ({ cookies }: { cookies: any[] }) => {
...
const FormsSection = ({ forms }: { forms: any[] }) => {
            ...
            {form.inputs?.length > 0 && (
              <div className="mt-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {form.inputs.slice(0, formLimits[i]).map((input: any, j: number) => (
...
const ApiCallsSection = ({ calls }: { calls: any[] }) => {
  if (!calls?.length) return null;

  const ApiCallCard = ({ call }: { call: any }) => {
